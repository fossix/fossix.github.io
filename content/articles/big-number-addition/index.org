...
- Input :: Pointer to two lists of single digits
- *Output* :: Pointer to a new list where each node is a digit of the summed value
  of two nodes from each of the input nodes.
